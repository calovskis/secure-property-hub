...
import { AlertCircle, ArrowRight, CheckCircle2, ClipboardList, MessageSquareQuote, PencilLine } from "lucide-react";
...
import { SellerResponsePanel } from "@/components/realtor/SellerResponsePanel";
...
export function AgreementTermsPanel({ leadId, propertyId, propertyLabel, purchase, buyerName, buyerEmail, agentName }: {
  ...
  return <div className={`mt-3 rounded-lg border-l-4 p-3 text-xs shadow-sm ${needsAgent ? "border-l-brand border border-border bg-background ring-1 ring-brand/20" : "border-l-border border border-border/60 bg-muted/40"}`}>
    ...
    {confirmed ? <><p className="mt-2 font-semibold text-success">Confirmed by the buyer — present to the seller</p><p className="mt-1 text-muted-foreground">Confirmed {plan?.termsConfirmedAt ? formatDateTime(plan.termsConfirmedAt) : ""}. Record the seller's answer below.</p><SellerResponsePanel leadId={leadId} propertyId={propertyId} propertyLabel={propertyLabel} purchase={purchase} buyerName={buyerName} buyerEmail={buyerEmail} agentName={agentName} /></> : null}
