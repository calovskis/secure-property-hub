    ...
    <div className="space-y-3 p-3">
      ...
      {plan?.sellerAgreedAt ? <>
        ...
        {plan.agreementUploadedAt ? <div className="space-y-2 rounded-md border border-success/40 bg-success/5 p-3">
          ...
          <p className="flex flex-wrap items-center gap-2 text-foreground"><FileText className="h-3.5 w-3.5 text-brand" />{plan.agreementDoc}<Button size="sm" variant="outline" className="h-7 gap-1 px-2 text-xs" onClick={() => { void downloadAgreementFile(leadId, plan.agreementDoc).then((ok) => { if (!ok) toast.error("The agreement copy could not be found. Please re-upload it."); }); }}><Download className="h-3 w-3" />Download copy</Button></p>
