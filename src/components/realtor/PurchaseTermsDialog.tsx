...
export function PurchaseTermsDialog({ open, onOpenChange, leadId, propertyLabel, purchase, buyerName, buyerEmail, agentName }: {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="flex max-h-[92vh] max-w-5xl flex-col gap-0 overflow-hidden p-0">
    ...
    <div className="flex flex-col-reverse gap-2 border-t border-border bg-background px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><Button type="button" variant="outline" disabled={step === 1} onClick={() => goTo(step - 1)}><ArrowLeft />Back</Button><div className="flex items-center justify-end gap-2"><span className="mr-2 hidden text-xs text-success sm:inline">Draft saved</span>{step < 4 ? <Button type="button" onClick={() => goTo(step + 1)}>Continue<ArrowRight /></Button> : alreadyConfirmed ? <Button type="button" onClick={() => onOpenChange(false)}><Check />Ready for seller</Button> : <Button type="button" disabled={!reviewed} onClick={sendToBuyer}><Check />Send to buyer for confirmation</Button>}</div></div>
